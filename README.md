...
* A `soundmap` directory that contains the source code - a fully executable WordPress plugin.
...
* rename files from `soundmap` to `example_me`
* change `soundmap` to `example_me`
* change `SOUNDMAP_` to `EXAMPLE_ME_`
...
Note that previous versions of the Boilerplate did not include `Soundmap_Loader` but this class is used to register all filters and actions with WordPress.
