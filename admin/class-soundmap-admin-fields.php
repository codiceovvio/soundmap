<?php
/**
 * The admin custom fields for the plugin.
 *
 ...
 * The admin custom fields for the plugin.
 *
 * This class defines all code necessary to add custom fields to
...
class Soundmap_Admin_Fields {
	 ...
	 * @since  0.1.1
	 * @access private
	 * @var    array $soundmap_options The options page for this plugin.
	 */
	...
	public function register_options_metabox( $soundmap_options ) {
		...
		*/
		$extra_tab_options->add_field( array(
			'desc'    => esc_html__( 'Google API key. To get an API key visit google.com', 'soundmap' ),
	...
	public static function soundmap_options_page_message_callback( $cmb, $args ) {
		...
		if ( ! empty( $args['should_notify'] ) ) {
			...
			if ( $args['is_updated'] ) {
				...
				// translators: %1$s is the settings page title, %2$s is the current active tab title.
